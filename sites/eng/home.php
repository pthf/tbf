<!DOCTYPE html>
<html>
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1">

	<title>Inicio | The Beer Fans | Social Network</title>

	<link rel="shortcut icon"  type="image/png" href="../../images/favicon.png">
	<link rel="stylesheet" type="text/css" href="../../styles/styles.css">
	<link rel="stylesheet" type="text/css" href="../../styles/styles_responsive.css">
	<link rel="stylesheet" href="http://code.jquery.com/mobile/1.2.0/jquery.mobile-1.2.0.min.css" />

	<script src="http://code.jquery.com/jquery-1.11.1.min.js"></script>
	<script type="text/javascript" src="../../js/all_pages_jquery.js"> </script>
	<script type="text/javascript" src="../../js/check.js"> </script>
	<script type="text/javascript" src="../../js/slider.js"> </script>
</head>
<body>

	<div id="menu_options">

		<div class="close_menu">
			<img src="../../images/close_image-01.png">
		</div>

		<div class="menu_list">
			<ul>
				<a href="home.html"><li><span>HOME</span></li></a>
				<a href=""><li><span>BEERS</span></li></a>
				<a href=""><li><span>PRODUCERS</span></li></a>
				<a href=""><li><span>RAW MATERIAL</span></li></a>
				<a href=""><li><span>MY PROFILE</span></li></a>
				<a href=""><li><span>SETTINGS</span></li></a>
				<a href=""><li class="no_border"><span>CLOSE</span></li></a>
				<a href=""><li class="no_border"><span>CLOSE</span></li></a>
			</ul>
		</div>

		<div class="social_other">
			<ul>
				<a href=""><li><img src="../../images/bottom-05.png"></li></a>
				<a href=""><li><img src="../../images/bottom-04.png"></li></a>
				<a href=""><li><img src="../../images/bottom-03.png"></li></a>
				<a href=""><li><img src="../../images/bottom-02.png"></li></a>
			</ul>
		</div>

	</div>

	 <div id="contenedor">

		<div class="top_info">
			<div class="contenedo_info">
				<a href="home.html">
				<div class="logo_tbf">
					<img src="../../images/menu_options-01.png" alt="The Beer Fans Logo" title="The Beer Fans Logo">
				</div>
				</a>
<!--
		<div class="perfil_tbf">

					<div class="search">
						<img src="../../images/icon-01.png" alt="search icon" title="search icon">
						<input type="text">
					</div>
					<div class="cont_info_user">
						<div class="msg">
							<a href="mensajes.html">
							<img src="../../images/menu_options-03.png" alt="icon message" title="icon message">
							</a>
						</div>
						<div class="profile_img">
							<a href="perfil.html">
							<img src="../../images/profile_default.jpg" alt="profile image" title="profile image">
							</a>
						</div>
						<div class="user_name">
							<span>USER NAME</span>
						</div>
						<div class="menu_buttom">
							<img src="../../images/menu_options-04.png" alt="menu image" title="menu image">
						</div>

					</div>
				</div>  -->
			</div>
		</div>

		<div class="top_img home">

		<!-- slider home -->
		<div id="slider">
			<figure>
				<!-- <img> = 1 slide -->
				<img src="../../images/banners/The Beer Fans Banner 3.png">
				<img src="../../images/banners/The Beer Fans Banner 1.png">
				<img src="../../images/banners/The Beer Fans Banner 2.png">
				<img src="../../images/banners/The Beer Fans Banner 4.png">
			</figure>
		</div>

		</div>

		<div class="cont_site">
			<div class="prin_img">
				<a href="">
				<img src="../../images/photo_pthf_home-04.png" alt="foto 1 principal tbf" title="foto 1 principal tbf">
				<div class="capa cap_eng">
					<span>BEERS</span>
				</div>
				</a>
			</div>

			<div class="prin_img">
				<a href="">
				<img src="../../images/photo_pthf_home-02.png" alt="foto 2 principal tbf" title="foto 2 principal tbf">
				<div class="capa cap_eng">
					<span>PRODUCERS</span>
				</div>
			</ar>
			</div>

			<div class="prin_img">
				<a href="">
				<img src="../../images/photo_pthf_home-03.png" alt="foto 3 principal tbf" title="foto 3 principal tbf">
				<div class="capa cap_eng">
					<span>RAW MATERIAL</span>
				</div>
				</a>
			</div>

			<div class="prin_img">
				<a href="http://www.thebeerfans.com/blog" target="_blank">
				<img src="../../images/NOTICIAS.png" alt="foto 4 principal tbf" title="foto 4 principal tbf">
				<div class="capa cap_eng" id="news">
					<span>NEWS</span>
				</div>
				</a>
			</div>

				<div class="slideshow">
				  <ul class="beers_month">

				    <li data-n="1">

							<img src="../../images/Beer Fans Banner Slider Sub 1.png" alt="tbf tarro" title="tbf tarro">
							<div class="aside_info_second">
								<div class="contenido_aside">
									<span class="title">HOPFENREICH</span>
									<span class="sub_title"> </span>
									<p>Es un nuevo bar dedicado a la cerveza artesanal que ha abierto en el vibrante Wrangelkiez de Kreuzberg. Catorce tipos distintos de cerveza de barril y una selección embotellada que se amplía por momentos. Escogen para su carta una combinación de marcas tradicionales con novedades y tendencias procedentes de todo el mundo.</p>
									<a href="http://www.thebeerfans.com/blog/2016/03/16/la-guia-definitiva-de-las-mejores-cervecerias-de-berlin/" target="_blank">
									<div class="boton_mas">READ MORE</div>
									</a>
								</div>
							</div>

						</li>

						<li data-n="2">

							<img src="../../images/Beer Fans Banner Slider Sub 2.png" alt="tbf tarro" title="tbf tarro">
							<div class="aside_info_second">
								<div class="contenido_aside">
									<span class="title">10 CERVEZAS ARTESANALES QUE DISTINGUIRÁS POR SU DISEÑO</span>
									<span class="sub_title"> </span>
									<p>Cerveza estilo Stout, de fermentación Ale. Aroma a cacao debido a que en su elaboración se utilizan maltas cafetosas y maltas chocolate perfectamente tostadas. Su color es de un negro intenso. Cerveza Minerva Stout Imperial es el orgullo de Minerva, sin duda el estandarte de la revolución cervecera en México.</p>
									<a href="http://www.thebeerfans.com/blog/2016/03/16/10-cervezas-artesanales-que-distinguiras-por-su-diseno/" target="_blank">
									<div class="boton_mas">READ MORE</div>
									</a>
								</div>
							</div>


						</li>

						<li data-n="3">
							<img src="../../images/Beer Fans Banner Slider Sub 3.png" alt="tbf tarro" title="tbf tarro">
							<div class="aside_info_second">
								<div class="contenido_aside">
									<span class="title">WHY SOME ENJOY BEER: WHAT YOU NEED TO KNOW</span>
									<span class="sub_title"> </span>
									<p>Cerveza estilo Stout, de fermentación Ale. Aroma a cacao debido a que en su elaboración se utilizan maltas cafetosas y maltas chocolate perfectamente tostadas. Su color es de un negro intenso. Cerveza Minerva Stout Imperial es el orgullo de Minerva, sin duda el estandarte de la revolución cervecera en México.</p>
									<a href="http://www.thebeerfans.com/blog/2016/03/16/why-some-enjoy-beer-what-you-need-to-know/" target="_blank">
									<div class="boton_mas">READ MORE</div>
									</a>
								</div>
							</div>


						</li>

						<!-- <li data-n="4">

							<img src="../../images/photo_pthf_home-03.png" alt="tbf tarro" title="tbf tarro">
							<div class="aside_info_second">
								<div class="contenido_aside">
									<span class="title">CUARTA CERVEZA</span>
									<span class="sub_title">CUARTA CERVEZA</span>
									<p>Cerveza estilo Stout, de fermentación Ale. Aroma a cacao debido a que en su elaboración se utilizan maltas cafetosas y maltas chocolate perfectamente tostadas. Su color es de un negro intenso. Cerveza Minerva Stout Imperial es el orgullo de Minerva, sin duda el estandarte de la revolución cervecera en México.</p>
									<a href="perfil_beer.html">
									<div class="boton_mas">READ MORE</div>
									</a>
								</div>
							</div>

						</li> -->

					</ul>

						<ul class="nav_beers">
						 <li data-cd="1"></li>
						 <li data-cd="2"></li>
						 <li data-cd="3"></li>
						 <!-- <li data-cd="4"></li> -->
 						</ul>

					</div>
<!--
				<div class="part_info_bottom">

					<div class="contact_us_or_follow">
						<img src="../../images/8150091.png" alt="tbf cervezas" title="tbf cervezas">
						<div class="contenido_usuarios">
							<span class="user_list">USUARIOS RECOMENDADOS</span>
							<div class="flex-container">
								<li class="flex-item"><img src="../../images/profile_default.jpg"/><span>USER 1</span></li>
								<li class="flex-item"><img src="../../images/profile_default.jpg"/><span>USER 2</span></li>
								<li class="flex-item"><img src="../../images/profile_default.jpg"/><span>USER 3</span></li>
								<li class="flex-item"><img src="../../images/profile_default.jpg"/><span>USER 4</span></li>
								<li class="flex-item"><img src="../../images/profile_default.jpg"/><span>USER 5</span></li>
								<li class="flex-item"><img src="../../images/profile_default.jpg"/><span>USER 6</span></li>
								<li class="flex-item"><img src="../../images/profile_default.jpg"/><span>USER 7</span></li>
								<li class="flex-item"><img src="../../images/profile_default.jpg"/><span>USER 8</span></li>
								<li class="flex-item"><img src="../../images/profile_default.jpg"/><span>USER 9</span></li>
								<li class="flex-item"><img src="../../images/profile_default.jpg"/><span>USER 10</span></li>
							</div>
						</div>
					</div>

					<div class="info_bottom">
						<img src="../../images/8150090.png" alt="tbf cervezas" title="tbf cervezas">
						<div class="aside_info_second first_part">
							<div class="contenido_aside">
								<span class="title">CONOCE A</span>
								<span class="sub_title">NAIPE</span>
								<p>Cerveza estilo Stout, de fermentación Ale. Aroma a cacao debido a que en su elaboración se utilizan maltas cafetosas y maltas chocolate perfectamente tostadas. Su color es de un negro intenso. Cerveza Minerva Stout Imperial es el orgullo de Minerva, sin duda el estandarte de la revolución cervecera en México.</p>
								<a href="perfil_beer.html">
								<div class="boton_mas">READ MORE</div>
								</a>
							</div>
						</div>
					</div>
				</div>
-->
				<div class="part_info_bottom">
					<div class="info_bottom">
						<img src="../../images/conoce2.png" alt="tbf cervezas" title="tbf cervezas">
						<div class="aside_info_second cont_part_less">
							<div class="contenido_aside">
								<span class="title">MEET</span>
								<span class="sub_title">NAIPE</span>
								<p>Cerveza estilo Stout, de fermentación Ale. Aroma a cacao debido a que en su elaboración se utilizan maltas cafetosas y maltas chocolate perfectamente tostadas. Su color es de un negro intenso. Cerveza Minerva Stout Imperial es el orgullo de Minerva, sin duda el estandarte de la revolución cervecera en México.</p>
								<a href="perfil_beer.html">
								<div class="boton_mas">READ MORE</div>
								</a>
							</div>
						</div>
					</div>
					<div class="contact_us_or_follow">
						<img src="../../images/8150091.png" alt="tbf cervezas" title="tbf cervezas">
						<div class="contenido_social">
							<span>CONTACT US OR FOLLOW US<br>ON OUR SOCIAL NETWORKS</span>
							<ul>
								<a href=""><li><img src="../../images/social-04.png"></li></a>
								<a href=""><li><img src="../../images/social-02.png"></li></a>
								<a href=""><li><img src="../../images/social-01.png"></li></a>
								<a href="contact.html"><li><img src="../../images/social-03.png"></li></a>
							</ul>
						</div>
					</div>
				</div>

		</div>

		<div class="bottom_site other_bottom">
			<div class="center_cont_site">
				<a href="#contenedor" class="ancla">
					<div class="go_top">
						<img src="../../images/go_top.png">
					</div>
				</a>
				<ul class="social">
					<a href=""><li><img src="../../images/bottom-05.png"></li></a>
					<a href=""><li><img src="../../images/bottom-04.png"></li></a>
					<a href=""><li><img src="../../images/bottom-03.png"></li></a>
					<a href=""><li><img src="../../images/bottom-02.png"></li></a>
				</ul>
				<ul class="nav">
<!--					<a href="home.html"><li><span>HOME</span></li></a>
					<a href="cervezas.html"><li><span>CERVEZAS</span></li></a>
					<a href="productores.html"><li><span>PRODUCTORES</span></li></a>
					<a href="materia.html"><li><spanRaw</span></li></a>
					<a href="perfil.html"><li><span>MI PERFIL</span></li></a>
					<a href="configuracion.html"><li><span>CONFIGURACIÓN</span></li></a> -->
					<a href="contact_en.html"><li><span>CONTACT</span></li></a>
				</ul>
				<span class="right_about">About Us - Política de Privacidad - FAQS</span>
				<span class="right_about">© 2015 The Beer Fans. All rights reserved.</span>
			</div>
		</div>

	</div>

</body>
</html>
